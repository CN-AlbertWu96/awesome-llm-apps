# 第一部分：导入必要的库
# 核心框架: Streamlit用于Web界面，Agno用于智能代理
# AI模型: Google Gemini作为主要的LLM
# 向量存储: Qdrant用于文档向量化存储
# 文档处理: LangChain用于PDF和网页内容处理
...
import google.generativeai as genai # AI模型
import bs4
from agno.agent import Agent # AI Agent用于智能代理
from agno.models.google import Gemini # AI模型
from langchain_community.document_loaders import PyPDFLoader, WebBaseLoader
from langchain.text_splitter import RecursiveCharacterTextSplitter # PDF及网页处理
from langchain_qdrant import QdrantVectorStore # 文档向量化存储
...
# ================================================================================================================================================
# 第二部分：自定义嵌入器类型
# 作用: 使用Gemini的text-embedding-004模型生成文档向量
# 维度: 768维向量空间
# 用途: 将文档转换为可搜索的向量表示
...
# 定义语义分割方法
# 定义循环分隔方法，包括单chunk字符长度，和chunk之间重叠字符长度
            ...
            with st.spinner("🤖 Thinking..."):
                try:
                    ...
                    if context:
                        ...
                        st.info(f"📊 Found context, and full prompt is {full_prompt}.")
                    else:
                        full_prompt = f"Original Question: {prompt}\nRewritten Question: {rewritten_query}"
                        st.info(f"ℹ️ No relevant information found in documents or web search, and full prompt is {full_prompt}.")
